...
import DemoOrderEntry from '../components/DemoOrderEntry';
import NiftyTicker from '../components/NiftyTicker';
import DemoPositionCard from '../components/DemoPositionCard';

// Standalone copy of TradingPage.tsx for anonymous/demo visitors - no
// useAuth(), no Profile/Admin/Logout/NotificationBell. Kept as a separate
// component (rather than an isDemo flag threaded through TradingPage) so
// the real trading page stays free of demo-only conditionals.
        ...
        <div className={`pnl-bar bg-${totalColor} bg-opacity-10 border-bottom`}>
          <Container className="py-2 d-flex justify-content-between align-items-center" style={{ position: 'relative' }}>
            <div className="d-flex align-items-center gap-2">
              <span className="fw-bold">Demo User</span>
              <Button variant="outline-secondary" size="sm" onClick={() => navigate('/')}>Exit Demo</Button>
            </div>
        ...
        <div className="mt-4">
          ...
          >
            ...
            >
              {trades.length > 0 ? (
                trades.map((trade) => (
                  <DemoPositionCard key={trade.tsym} trade={trade} />
                ))
              ) : (
            ...
            >
              {closedTrades.length > 0 ? (
                closedTrades.map((trade, i) => (
                  <DemoPositionCard key={`${trade.tsym}-closed-${i}`} trade={trade} closed />
                ))
              ) : (
                <div className="text-center text-muted py-4">
