...
class PositionCard extends ConsumerWidget {
  ...
  Widget build(BuildContext context, WidgetRef ref) {
    ...
    PositionState data = ref.watch(positionProvider);
    TradeServiceNotifier tradeServiceNotifier = ref.read(tradeServiceProvider.notifier);
    ...
    return _getScreenWidget(context, data, tradeServiceNotifier);
  ...
  Widget _getScreenWidget(BuildContext context, PositionState data,
      TradeServiceNotifier notifier) {
    // debugPrint('Position Data: ${data.positions}');
    List<Widget> cards = [];
    for (Position position in data.positions) {
      cards.add(_getPositionCard(context, position, notifier));
    }
  ...
  Widget _getRight(Position data) =>
      Row(mainAxisAlignment: MainAxisAlignment.start, children: [
        Padding(
          ...
          child: Text(
            data.right, style: TextStyle(fontSize: 24),
          ),
        ),
      ]);
      ...
      child: Column(
        ...
        children: <Widget>[
          ...
          (ElevatedButton(
              onPressed: () {
                notifier.squareOff(
                    data.token, data.right, data.quantity);
              },
              child: const Text('Square Off', style: TextStyle(fontSize: 18),)))
        ],
      ),
    );
  }
