...
class NiftyCard extends ConsumerWidget {
  ...
  Widget build(BuildContext context, WidgetRef ref) {
    ...
    TradeServiceNotifier tradeNotifier = ref.read(tradeServiceProvider.notifier);
  ...
  Widget _getScreenWidget(
      context, data, TradeServiceNotifier tradeNotifier) {
    return _getNiftyCard(context, data, tradeNotifier);
  }
          ...
          child: Column(
            ...
            children: <Widget>[
              ...
              Padding(
                  ...
                  child:
                      getActionButtons(tradeNotifier, data.niftyQuote.ltp)),
            ],
          ),
        ));
  }
  ...
  Widget getActionButtons(TradeServiceNotifier tradeNotifier, num ltp) =>
      Row(
          ...
          children: [
            SizedBox(
                ...
                child: ElevatedButton(
                    onPressed: () async {
                      await tradeNotifier.buy('NIFTY', 'call');
                    },
                    child: const Text('Up'))),
            SizedBox(
                width: 150,
                child: ElevatedButton(
                    onPressed: () async {
                      await tradeNotifier.buy('NIFTY', 'put');
                    },
                    child: const Text('Down'))),
          ]);

  Widget _getRefresh(TradeServiceNotifier positionNotifier, NiftyState data) =>
...
class VariableCard extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    final TextStyle greenStyle =
        Theme.of(context).textTheme.labelLarge!.apply(color: Colors.green, fontSizeFactor: 1.2);
    final TextStyle redStyle =
        Theme.of(context).textTheme.labelLarge!.apply(color: Colors.red, fontSizeFactor: 1.2);
    return Card(
        child: Center(
